...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    ...
    if (self) {
        ...
        __tableViewStyle = UITableViewStylePlain;
    }
    return self;
}
...
- (void)viewDidLoad {
    [super viewDidLoad];
    __tableView = [[UITableView alloc] initWithFrame:self.view.bounds style:__tableViewStyle];
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    ...
    if ([__infoPanel superview] == nil) {
        ...
        [UIView animateWithDuration:KNPathTableFadeInDuration animations:^{
            __infoPanel.alpha = 1;
            __infoPanel.frame = f;
...
- (void)slideOutInfoPanel {
    ...
    [UIView animateWithDuration:KNPathTableFadeOutDuration animations:^{
        __infoPanel.alpha = 0;
        __infoPanel.frame = f;
    }                completion:^(BOOL finished) {
        [__infoPanel removeFromSuperview];
